...
* `device_id`: this is the device id combined with the platform id: `device_id | (platform_id << 32)`. The platform id is 4 on 3DS and 5 on Wii U. On Wii U the device id is stored in the OTP and can be retrieved with `MCP_GetDeviceId`.