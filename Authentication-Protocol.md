...
All game servers have a bunch of special users. Normally, the password of the guest account is `MMQea3n!fsik`, but Nintendo seems to have changed this password on all Wii U and 3DS servers. It used to be possible to log in as guest on Switch servers as well, but the guest account has been deleted by now.
