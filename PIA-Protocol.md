...
### Switch
If encryption is enabled, The [payload](#payload) is encrypted with AES-GCM. The authentication tag is stored in the [header](#header). No other signature is appended to the packet. The nonce is generated by appending the nonce in the [header](#header) to 4 bytes that depend on the network type.

* **NEX mode:** `(gathering_id & 0xFFFFFF) | (connection_id << 24)`
* **LDN mode:** unknown
* **LAN mode:** unknown